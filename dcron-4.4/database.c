...
SynchronizeDir(const char *dpath, const char *user_override, int initial_scan)
{
	...
	if (DebugOpt)
		printlogf(LOG_DEBUG, "SynchronizeDir %s\n", dpath);
	...
	if (DebugOpt)
		printlogf(LOG_DEBUG, "deleted all DB files in %s\n", dpath);
	...
	if ((dir = opendir(dpath)) != NULL) {
		while ((den = readdir(dir)) != NULL) {
			printlogf(LOG_DEBUG, "den->d_name==%s\n", den->d_name);
			if (strchr(den->d_name, '.') != NULL) {
				continue;
				}
			if (strcmp(den->d_name, CRONUPDATE) == 0) {
				printlogf(LOG_DEBUG, "CRONUPDATE %s\n", den->d_name);
				continue;
				}
			if (user_override) {
				printlogf(LOG_DEBUG, "user_override %s\n", den->d_name);
				SynchronizeFile(dpath, den->d_name, user_override);
			} else if (getpwnam(den->d_name)) {
				printlogf(LOG_DEBUG, "SynchronizeFile den->d_name %s\n", den->d_name);
				SynchronizeFile(dpath, den->d_name, den->d_name);
			} else {
				printlogf(LOG_DEBUG, "ignoring %s/%s (non-existent user)\n",
						dpath, den->d_name);
			}
		}
		closedir(dir);
	} else {
		printlogf(LOG_DEBUG, "dir = opendir(dpath)) == NULL\n" );
		if (initial_scan)
...
ReadTimestamps(const char *user)
{
	...
	printlogf(LOG_ERR, "ReadTimestamps\n");
	file = FileBase;
	while (file != NULL) {
		printlogf(LOG_ERR, "ReadTimestamps (user %s)\n", file->cf_UserName);
...
SynchronizeFile(const char *dpath, const char *fileName, const char *userName)
{
	 ...
	 * Limit entries
	 */
	printlogf(LOG_ERR, "SynchronizeFile\n");
	...
	if ((fi = fopen(path, "r")) != NULL) {
		printlogf(LOG_DEBUG, "SynchronizeFile: fopen(path, r..\n");
		struct stat sbuf;

		printlogf(LOG_DEBUG, "fstat(fileno(fi), &sbuf) == %i \n", fstat(fileno(fi), &sbuf));
		printlogf(LOG_DEBUG, "sbuf.st_uid == %u, DaemonUid== %u\n", sbuf.st_uid, DaemonUid);
		//if (fstat(fileno(fi), &sbuf) == 0 && sbuf.st_uid == DaemonUid) {
		if (fstat(fileno(fi), &sbuf) == 0) {
			printlogf(LOG_DEBUG, "fstat(fileno(fi), &sbuf) == 0 && sbuf.st_uid == DaemonUid \n");
...
TestJobs(time_t t1, time_t t2)
{
	...
	printlogf(LOG_DEBUG, "TestJobs\n");
	for (file = FileBase; file; file = file->cf_Next) {
		printlogf(LOG_DEBUG, "Inside TestJobs\n");
		if (file->cf_Deleted)
			continue;
	...
	printlogf(LOG_DEBUG, "TestJobs: NO for File...\n");
	/*
	 * Find jobs > t1 and <= t2
	 */
...
TestStartupJobs(void)
{
	...
	printlogf(LOG_DEBUG, "TestStartupJobs\n");
	for (file = FileBase; file; file = file->cf_Next) {
		if (DebugOpt)
...
CheckJobs(void)
{
	...
	printlogf(LOG_DEBUG, "CheckJobs\n");
	for (file = FileBase; file; file = file->cf_Next) {
		printlogf(LOG_DEBUG, "CheckJobs %s\n", file->cf_FileName);
		if (file->cf_Running) {
			file->cf_Running = 0;
