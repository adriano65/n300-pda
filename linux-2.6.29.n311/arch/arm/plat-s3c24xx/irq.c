...
static void s3c_irq_mask(unsigned int irqno) {
	unsigned long mask;
	//if (irqno==42) printk("s3c_irq_mask 42 ----------------------------\n");
	irqno -= IRQ_EINT0;
...
static inline void s3c_irq_ack(unsigned int irqno) {
	...
	//if (irqno==42) { bitval = ~(0x04000000); printk("s3c_irq_ack 42 , bitval==0x%lx\n", bitval); }
	//if (irqno==42) printk("s3c_irq_ack 42 , bitval==0x%lx\n", bitval);
	//if (irqno==42) { bitval = ~(0x04000000); bitval=0; }		//cxlabs
...
s3c_irq_maskack(unsigned int irqno)
{
	...
	//if (irqno==42) { bitval = ~(0x04000000); }		//cxlabs
	__raw_writel(bitval, S3C2410_SRCPND);
	__raw_writel(bitval, S3C2410_INTPND);
}
...
void __init s3c24xx_init_irq(void)
{
	...
	for (irqno = IRQ_EINT4t7; irqno <= IRQ_ADCPARENT; irqno++) {
		...
		default:
			//printk("registering irq %d (s3c irq)\n", irqno);
