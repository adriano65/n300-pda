...
static int nand_block_bad(struct mtd_info *mtd, loff_t ofs, int getchip)
{
	...
	if (chip->options & NAND_BUSWIDTH_16) {
		...
		//DPRINTK("before chip->cmdfunc %p, chip->badblockpos 0x%04X\n", chip->cmdfunc, chip->badblockpos);
		chip->cmdfunc(mtd, NAND_CMD_READOOB, chip->badblockpos, page);
		if (chip->read_byte(mtd) != 0xff) {
			DPRINTK("chip->read_byte(mtd) != 0xff\n");
			res = 1;
		}
	}
...
static void nand_command(struct mtd_info *mtd, unsigned int command, int column, int page_addr) {
	...
	if (page_addr != -1) {
		...
		ctrl &= ~NAND_CTRL_CHANGE;
		DPRINTK("page_addr >> 8 0x%02X\n", page_addr >> 8);
		chip->cmd_ctrl(mtd, page_addr >> 8, ctrl);
		/* One more address cycle for devices > 32MiB */
		if (chip->chipsize > (32 << 20))
			chip->cmd_ctrl(mtd, page_addr >> 16, ctrl);
	}
	DPRINTK("command NAND_CMD_NONE 0x%02X\n", NAND_CMD_NONE);
...
static int nand_do_read_ops(struct mtd_info *mtd, loff_t from, struct mtd_oob_ops *ops) {
	...
	DPRINTK(" mtd->ecc_stats.corrected - stats.corrected=0x%04X\n", mtd->ecc_stats.corrected - stats.corrected);
...
static int nand_do_read_oob(struct mtd_info *mtd, loff_t from, struct mtd_oob_ops *ops)
{
	...
	if (unlikely(ops->ooboffs >= len)) {
		DPRINTK("Attempt to start read outside oob\n");
		return -EINVAL;
	}
	...
	if (unlikely(from >= mtd->size ||
		...
		DPRINTK("Attempt read beyond end of device\n");
		return -EINVAL;
	}
...
static int nand_do_write_ops(struct mtd_info *mtd, loff_t to, struct mtd_oob_ops *ops)
{
	...
	if (NOTALIGNED(to) || NOTALIGNED(ops->len)) {
		DPRINTK("Attempt to write not page aligned data (EINVAL)\n");
		return -EINVAL;
	}
	...
	if (nand_check_wp(mtd)) {
		DPRINTK("Device is write protected (EIO)\n");
		return -EIO;
	  }
...
static int nand_write(struct mtd_info *mtd, loff_t to, size_t len, size_t *retlen, const uint8_t *buf)
{
	...
	DPRINTK("to 0x%08X, len 0x%08X, mtd->size 0x%08X\n", to, len, mtd->size);
	/* Do not allow reads past end of device */
	if ((to + len) > mtd->size) {
		DPRINTK("EINVAL\n");
		return -EINVAL;
			     ...
			     struct mtd_oob_ops *ops)
{
	...
	if (unlikely(ops->ooboffs >= len)) {
		DPRINTK("Attempt to start write outside oob\n");
		return -EINVAL;
	}
	...
	if (unlikely(to >= mtd->size ||
		...
		DPRINTK("Attempt write beyond end of device\n");
		return -EINVAL;
	}
...
int nand_erase_nand(struct mtd_info *mtd, struct erase_info *instr,  int allowbbt)
{
	...
	DPRINTK("start = 0x%08x, len = %i\n", (unsigned int) instr->addr, (unsigned int) instr->len);

	/* Start address must align on block boundary */
	if (instr->addr & ((1 << chip->phys_erase_shift) - 1)) {
		DPRINTK(" Unaligned address\n");
		return -EINVAL;
	}

	/* Length must align on block boundary */
	if (instr->len & ((1 << chip->phys_erase_shift) - 1)) {
		DPRINTK (" Length not block aligned\n");
		return -EINVAL;
	}

	/* Do not allow erase past end of device */
	if ((instr->len + instr->addr) > mtd->size) {
		DPRINTK (" Erase past end of device\n");
		return -EINVAL;
	}
	...
	/* Check, if it is write protected */
	if (nand_check_wp(mtd)) {
		DPRINTK (" Device is write protected!!!\n");
	...
	while (len) {
		...
		if (status & NAND_STATUS_FAIL) {
			DPRINTK("Failed erase, page 0x%08x\n", page);
			instr->state = MTD_ERASE_FAILED;
 ...
 erase_exit:
	...
	for (chipnr = 0; chipnr < chip->numchips; chipnr++) {
		...
		DPRINTK(" nand_update_bbt (%d:0x%0x 0x%0x)\n", chipnr, rewrite_bbt[chipnr],
						  ...
						  int busw, int *maf_id)
{
	...
	DPRINTK("Send NAND_CMD_READID command\n");
	chip->cmdfunc(mtd, NAND_CMD_READID, 0x00, -1);

	DPRINTK("Read manufacturer and device IDs\n");
	*maf_id = chip->read_byte(mtd);
	dev_id = chip->read_byte(mtd);

	DPRINTK("maf_id 0x%02X dev_id 0x%02X\n",*maf_id,dev_id);
	/* Lookup the flash id */
	...
	if (!type->pagesize) {
		...
		/* mfr 0xEC, id 0xF1, id2 0x80, id3 0x15, id4 0x40 */
		chip->cellinfo = chip->read_byte(mtd);
		/* The 4th id byte is the important one */
		uint8_t extid, extid_dbg;
		extid_dbg = extid = chip->read_byte(mtd);
		/* Calc pagesize */
		...
		DPRINTK("chip->cellinfo 0x%02X, extid 0x%02X, mtd->writesize 0x%04X, mtd->oobsize 0x%02X\n", chip->cellinfo, extid_dbg, mtd->writesize,mtd->oobsize);
	} else {
		/*
		 * Old devices have chip data hardcoded in the device id table (nand_ids.h)
		 */
	...
	DPRINTK("NAND device: Manufacturer ID: 0x%02x, Chip ID: 0x%02x (%s %s)\n", *maf_id, dev_id,
