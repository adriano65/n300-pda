...
struct nand_chip nand_chips[CFG_MAX_NAND_DEVICE] = {{0}};
//struct nand_manufacturers nand_manuf_ids[CFG_MAX_NAND_DEVICE];
...
static int NanD_IdentChip(struct nand_chip *nand_chip, int floor, int chip) {
	int mfr, id, id2, id3, id4, i;

	NAND_ENABLE_CE(nand_chip);  /* set pin low */
	/* Reset the chip */
	if (NanD_Command(nand_chip, NAND_CMD_RESET)) {
		DPRINTK("NanD_Command (reset) for %d,%d returned true\n", floor, chip);
		NAND_DISABLE_CE(nand_chip);  /* set pin high */
		return 0;
	}

	/* Read the NAND chip ID: 1. Send ReadID command */
	if (NanD_Command(nand_chip, NAND_CMD_READID)) {
	...
	NanD_Address(nand_chip, ADDR_COLUMN, 0);

	/* Read the manufacturer and device id codes from the device */

	mfr = READ_NAND(nand_chip->IO_ADDR);
	id = READ_NAND(nand_chip->IO_ADDR);
	id2 = READ_NAND(nand_chip->IO_ADDR);
	id3 = READ_NAND(nand_chip->IO_ADDR);
	id4 = READ_NAND(nand_chip->IO_ADDR);

	NAND_DISABLE_CE(nand_chip);  /* set pin high */

	DPRINTK("NanD_Command (ReadID) got mfr 0x%02X, id 0x%02X, id2 0x%02X, id3 0x%02X, id4 0x%02X\n", mfr, id, id2, id3, id4);
	if (mfr == 0xff || mfr == 0) {
	...
	if (nand_manuf_ids->name) {
		if (nand_manuf_ids->name == mfr && nand_manuf_ids->id == id) {
	...
	for (i = 0; nand_flash_devs[i].name != NULL; i++) {
		if (mfr == nand_flash_devs[i].name &&
		    id == nand_flash_devs[i].id) {
			printf("Flash chip found:\n\t Manufacturer ID: 0x%2.2X, Chip ID: 0x%2.2X (%s)\n", mfr, id,
			       nand_flash_devs[i].name);
			if (!nand_manuf_ids->name) {
				nand_manuf_ids->name = mfr;
				nand_manuf_ids->id = id;
				nand_chip->chipshift = nand_flash_devs[i].chipshift;
				nand_chip->page256 = nand_flash_devs[i].page256;
				nand_chip->eccsize = 256;
				if (nand_chip->page256) {
					nand_chip->oobblock = 256;
					nand_chip->oobsize = 8;
					nand_chip->page_shift = 8;
				} else {
					nand_chip->oobblock = 512;
					nand_chip->oobsize = 16;
					nand_chip->page_shift = 9;
				}
				nand_chip->pageadrlen = nand_flash_devs[i].pageadrlen;
				nand_chip->erasesize  = nand_flash_devs[i].erasesize;
				nand_chip->chips_name = nand_flash_devs[i].name;
				nand_chip->bus16	 = nand_flash_devs[i].bus16;
 				return 1;
			}
			return 0;
		}
...
static void NanD_ScanChips(struct nand_chip *nand_chip) {
	int floor, chip;
	int numchips[NAND_MAX_FLOORS];
	int maxchips = NAND_MAX_CHIPS;
	int ret = 1;

	nand_chip->numchips = 0;
	nand_chip->mfr = 0;
	nand_manuf_ids->id = 0;
	...
	for (floor = 0; floor < NAND_MAX_FLOORS; floor++) {
		...
		for (chip = 0; chip < maxchips && ret != 0; chip++) {
			...
			ret = NanD_IdentChip(nand_chip, floor, chip);
			if (ret) {
				numchips[floor]++;
				nand_chip->numchips++;
			}
...
#ifdef NAND_DEBUG
		puts ("No NAND flash chips recognized.\n");
#endif
		return;
	}

	/* Allocate an array to hold the information for each chip */
	nand_chip->chips = malloc(sizeof(struct Nand) * nand_chip->numchips);
	if (!nand_chip->chips) {
	...
	for (floor = 0; floor < NAND_MAX_FLOORS; floor++) {
		for (chip = 0; chip < numchips[floor]; chip++) {
			nand_chip->chips[ret].floor = floor;
			nand_chip->chips[ret].chip = chip;
			nand_chip->chips[ret].curadr = 0;
			nand_chip->chips[ret].curmode = 0x50;
			ret++;
		}
	}

	/* Calculate and print the total size of the device */
	nand_chip->totlen = nand_chip->numchips * (1 << nand_chip->chipshift);
...
unsigned long nand_probe(unsigned long physadr) {
	...
	for (i=0; i<CFG_MAX_NAND_DEVICE; i++) {
		if (nand_chips[i].ChipID == NAND_ChipID_UNKNOWN) {
			nand_chip = &nand_chips[i];
			break;
		}
	}
	if (!nand_chip)
	...
	nand_chip->IO_ADDR = physadr;
	nand_chip->cache_page = -1;  /* init the cache page */
	NanD_ScanChips(nand_chip);

	if (nand_chip->totlen == 0) {
		/* no chips found, clean up and quit */
		memset((char *)nand_chip, 0, sizeof(struct nand_chip));
		nand_chip->ChipID = NAND_ChipID_UNKNOWN;
		return (0);
	}

	nand_chip->ChipID = ChipID;
	//if (curr_device == -1)
	//	curr_device = i;

	nand_chip->data_buf = malloc (nand_chip->oobblock + nand_chip->oobsize);
	if (!nand_chip->data_buf) {
		puts ("Cannot allocate memory for data structures.\n");
		return (0);
	}

	return (nand_chip->totlen);
}
...
void nand_init(void) {
	...
	nand->NFCONT &= ~(1<<1); 			//NFCE_LOW
	nand->NFCMD = 0xFF; 				// reset command
	for(i = 0; i < 10; i++); 			// tWB = 100ns.
	while (!(nand->NFSTAT & (1<<0)));	// NF_WaitRB(); // wait 200~500us;
	nand->NFCONT |= (1<<1);				//NFCE_HIGH
	//puts ("NFCE_HIGH:  \n");
...
static void nand_cmdfunc(struct mtd_info *mtd, unsigned command, int column, int page_addr)
{
...
#else
 #error "U-Boot legacy NAND support not available for N300."
#endif
#endif
