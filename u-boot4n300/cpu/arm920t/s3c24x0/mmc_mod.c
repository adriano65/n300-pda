...
u_int32_t *mmc_cmd(ushort opcode, ulong arg, ushort flags) {
	...
	switch (opcode) {
	  ...
	  case MMC_CMD_SET_RELATIVE_ADDR:
		sprintf(sCmd,"MMC_CMD_SET_RELATIVE_ADDR (%u)", opcode);
		break;
	  ...
	  case MMC_STOP_TRANSMISSION:
		sprintf(sCmd,"MMC_STOP_TRANSMISSION (%u)", opcode);
		break;
...
int mmc_block_read(uchar *dst, ulong src, ulong len) {
	...
	// ??? dcon = (len >> 11) & S3C2410_SDIDCON_BLKNUM;
	...
	//resp = mmc_cmd(MMC_CMD_READ_BLOCK, ((mmc_dev.if_type == IF_TYPE_SDHC) ? (src >> 9) : src)+1, CMD_F_RESP);
	//resp = mmc_cmd(MMC_CMD_READ_BLOCK, 1, CMD_F_RESP);
	...
	while (len > 0) {
		...
		//fifo = FIFO_FILL(sdi);
		while (fifo--) {
			//pr_debug("dst_u32 = 0x%08x, *dst_u32 = 0x%08x, sdi->SDIDCNT = 0x%04x \n", dst_u32, *dst_u32, sdi->SDIDCNT,sdi->SDIFSTA);
			*(dst_u32++) = sdi->SDIDAT;
			if (len >= 4)
				len -= 4;
			else {
