...
static void s3c2410_nand_select_chip(struct mtd_info *mtd, int chip)
{
	...
	DPRINTK("info->sel_reg 0x%08lX, cur is 0x%08lX\n", info->sel_reg, cur);
...
static void s3c2440_nand_hwcontrol(struct mtd_info *mtd, int cmd, unsigned int ctrl)
{
	...
	if (ctrl & NAND_CLE) {
		DPRINTK("ctrl & NAND_CLE, cmd is 0x%04X\n", cmd);
		writeb(cmd, info->regs + S3C2440_NFCMD);
		}
	else {
		DPRINTK("cmd (address) is 0x%04X\n", cmd);
...
static int s3c2440_nand_devready(struct mtd_info *mtd) {
	unsigned char nS3C2440_NFSTAT;
	struct s3c2410_nand_info *info = s3c2410_nand_mtd_toinfo(mtd);
	nS3C2440_NFSTAT=readb(info->regs + S3C2440_NFSTAT);
	DPRINTK("S3C2440_NFSTAT is 0x%04X\n", nS3C2440_NFSTAT);
	return nS3C2440_NFSTAT & S3C2440_NFSTAT_READY;
}
				     ...
				     u_char *read_ecc, u_char *calc_ecc)
{
	...
	DPRINTK("%s: rd %02x%02x%02x calc %02x%02x%02x diff %02x%02x%02x\n",
		 __func__,
	...
	if (((diff0 ^ (diff0 >> 1)) & 0x55) == 0x55 &&
		...
		DPRINTK("correcting error bit %d, byte %d\n", bit, byte);
...
static int s3c2410_nand_calculate_ecc(struct mtd_info *mtd, const u_char *dat, u_char *ecc_code)
{
	...
	DPRINTK("returning ecc %02x%02x%02x\n", ecc_code[0], ecc_code[1], ecc_code[2]);
...
static int s3c2412_nand_calculate_ecc(struct mtd_info *mtd, const u_char *dat, u_char *ecc_code)
{
	...
	DPRINTK("calculate_ecc: returning ecc %02x,%02x,%02x\n", ecc_code[0], ecc_code[1], ecc_code[2]);
...
static int s3c24xx_nand_probe(struct platform_device *pdev, enum s3c_cpu_type cpu_type)
{
	...
	DPRINTK("ioremap registers at %p, size %x\n", info->regs, size);
	...
	for (setno = 0; setno < nr_sets; setno++, nmtd++) {
		DPRINTK("initializing set %d (%p, info %p)\n", setno, nmtd, info);
